...
export const Inventory: React.FC = () => {
  ...
  const YIELD_CACHE_KEY = 'inventory_yield_cache_v1';

  // Guard refs: ngăn nhiều fetch chạy song song và tránh timeout vô hạn
  ...
  useEffect(() => {
    try {
      ...
      const cachedYield = localStorage.getItem(YIELD_CACHE_KEY);
      if (cachedYield) {
        const parsedYield = JSON.parse(cachedYield);
        if (Array.isArray(parsedYield) && parsedYield.length > 0) {
          setMenuItems(parsedYield);
        }
      }
    } catch (e) {
      console.warn('[Inventory] invalid cache, ignore:', e);
    }
  }, []);

  // BUGFIX: fetchData định nghĩa TRƯỚC useEffect để tránh stale closure.
  // Nếu fetchData đứng SAU useEffect, React closure của useEffect dùng
  // phiên bản fetchData từ render đầu tiên — khi viewMode đổi, fetchData
  // được tạo lại nhưng useEffect không biết → isFetching.current từ closure
  // cũ vẫn là true → guard block toàn bộ fetch mới → loading stuck.
  const fetchData = useCallback(async () => {
    // Guard: nếu đang fetch rồi thì bỏ qua, tránh chạy song song
    ...
    // Timeout guard: sau 15 giây tự động tắt loading và hiện lỗi
    timeoutRef.current = setTimeout(() => {
        if (isFetching.current) {
            isFetching.current = false;
            setLoading(false);
            setErrorMsg('Kết nối quá thời gian. Kiểm tra internet và thử lại.');
        }
    }, 15000);

    try {
        // Fetch Inventory
        const { data: invData, error } = await withTimeout(
          supabase.from('inventory').select('*').order('name') as any,
          10000,
        ...
        try { localStorage.setItem(INVENTORY_CACHE_KEY, JSON.stringify(processedInv)); } catch {}

        // Fetch Menu Items & Yields if in Yield Mode
        if (viewMode === 'yield') {
            const { data: menuData } = await withTimeout(
              supabase
                .from('menu_items')
                .select('*, menu_item_ingredients(*)') as any,
              10000,
              'inventory:fetchYieldMenu'
            ) as any;
            ...
            if (menuData) {
                ...
                try { localStorage.setItem(YIELD_CACHE_KEY, JSON.stringify(yieldCalculated)); } catch {}
            }
        }
    } catch (error: any) {
        console.error('Error fetching data:', error);
        // Hiện lỗi cụ thể thay vì chỉ "Failed to load data"
        if (error.message?.includes('relation') || error.message?.includes('does not exist')) {
            setErrorMsg('Bảng "inventory" chưa được tạo trong Supabase. Vui lòng kiểm tra database.');
        } else if (error.message?.includes('JWT') || error.message?.includes('auth')) {
            setErrorMsg('Phiên đăng nhập hết hạn. Vui lòng đăng nhập lại.');
        } else {
            setErrorMsg(error.message || 'Không thể tải dữ liệu. Vui lòng thử lại.');
        }
    } finally {
        // Dọn dẹp timeout và đặt lại guard
  ...
  // BUGFIX: fetchData nằm trong deps để luôn dùng instance mới nhất.
  // isFetching reset tường minh ở đây để đảm bảo fetch mới luôn được phép
  // chạy khi viewMode hoặc fetchData thay đổi — không phụ thuộc cleanup của effect trước.
  useEffect(() => {
    isFetching.current = false; // Reset guard, cho phép fetch mới chạy
    ...
    // Cleanup khi unmount hoặc deps thay đổi giữa chừng
    return () => {
        isFetching.current = false;
        if (timeoutRef.current) clearTimeout(timeoutRef.current);
    };
  ...
  const hasAnyRows = items.length > 0 || menuItems.length > 0;
  const showBlockingLoading = loading && !hasAnyRows;
