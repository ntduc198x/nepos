...
import { printOrderReceipt, generateReceiptHTML, isSandboxed } from '../services/printRuntime';
...
export const Dashboard: React.FC = () => {
  ...
  const [deviceBottomInset, setDeviceBottomInset] = useState(0);
  ...
  // BUGFIX C: Trì hoãn 3 giây trước khi Dashboard tự refresh
  // DataContext đã gọi refreshOrdersForDashboard trong refreshData() khi mount.
  // Nếu Dashboard cũng gọi ngay lập tức, cả hai sẽ race với isPulling mutex
  // → caller thứ hai bị block hoàn toàn, không lấy được dữ liệu gì cả.
  // Delay 3s đảm bảo DataContext's initial fetch xong trước, sau đó Dashboard
  // có thể poll để cập nhật dữ liệu mới nhất.
  useEffect(() => {
  ...
  const showInitialLoading = dataLoading && orders.length === 0 && menuItems.length === 0;

  if (showInitialLoading) return <div className="flex h-full items-center justify-center"><Loader2 className="animate-spin text-primary" size={48} /></div>;
        ...
        {!viewingOrder ? (
          ...
          <>
            ...
            <div 
              ...
              style={{
                bottom: 'clamp(calc(env(safe-area-inset-bottom, 0px) + 56px), var(--app-bottom-offset, calc(env(safe-area-inset-bottom, 0px) + 96px)), calc(env(safe-area-inset-bottom, 0px) + 88px))'
              }}
            >
               {(() => {
                  const { items: enriched, totalAmount: subtotal } = enrichOrderDetails(viewingOrder, menuItems);
