...
const UserPinManagement: React.FC<{ isAdmin: boolean; isManager: boolean }> = ({ isAdmin, isManager }) => {
  const { t } = useTheme();
  ...
  const fetchUsers = async () => {
    ...
    // Always seed at least current logged-in account so PIN can be set offline.
    const seedFromCurrentUser = () => {
      if (!authUser?.email) return false;
      setUsers(prev => {
        if (prev.some(u => String(u.email).toLowerCase() === String(authUser.email).toLowerCase())) return prev;
        return [{
    ...
    try {
      // 1. Load local registry first
      const registry: any[] = safeLoadRegistry();
      if (registry.length > 0) {
        setUsers(registry.map((r: any) => ({
          id: r.auth_user_id || r.email,
          email: r.email,
          role: r.role || 'staff',
          pin_code: r.pin,
          full_name: r.email?.split('@')?.[0] || 'Local User'
        })));
      } else {
        seedFromCurrentUser();
      }
      ...
      const { data, error } = await withTimeout(
        supabase.from('users').select('*').order('created_at', { ascending: true }) as any,
        10000,
        'fetchUsers'
      ) as any;
      ...
      if (data) {
        // 3. Merge Supabase data with local registry
        const mergedUsers = data.map(u => {
            const local = registry.find(r => r.email === u.email);
            return {
                ...u,
                pin_code: u.pin_code || (local ? local.pin : null)
            };
        });
        setUsers(mergedUsers);

        // 4. Update registry with any new pins from Supabase
        const newRegistry = mergedUsers.map(u => ({
            role: u.role || 'staff',
            email: u.email,
            pin: u.pin_code,
            auth_user_id: u.id
        })).filter(u => u.pin);
        localStorage.setItem(POS_PIN_REGISTRY_KEY, JSON.stringify(newRegistry));
      }
    } catch (e: any) { 
        console.error("Fetch users failed:", e);
        // Fallback to registry (safe parse)
        const registry = safeLoadRegistry();
        if (registry.length > 0) {
            setUsers(registry.map((r: any) => ({
                id: r.auth_user_id || r.email,
                email: r.email,
            ...
            const hasCurrent = seedFromCurrentUser();
            setErrorMsg(hasCurrent ? '' : (e.message || 'Timeout/RLS denied'));
        }
    } 
    finally {
      setLoading(false);
  ...
  // Thêm timeout guard:
  useEffect(() => {
    if (isAdmin || isManager) fetchUsers(); // Manager cũng nên được xem
  }, [isAdmin, isManager]);
  ...
  const handleUpdatePin = async () => {
    ...
    // 1. Update local registry immediately (Primary source of truth now)
    try {
      const rawRegistry = localStorage.getItem(POS_PIN_REGISTRY_KEY);
      let registry = rawRegistry ? JSON.parse(rawRegistry) : [];
      const userIdx = registry.findIndex((u: any) => u.email === editingUser.email);
      if (userIdx > -1) {
          registry[userIdx].pin = newPin;
          registry[userIdx].role = editingUser.role || registry[userIdx].role;
          registry[userIdx].auth_user_id = editingUser.id || registry[userIdx].auth_user_id;
      } else {
          registry.push({ role: editingUser.role || 'staff', email: editingUser.email, pin: newPin, auth_user_id: editingUser.id });
      }
      localStorage.setItem(POS_PIN_REGISTRY_KEY, JSON.stringify(registry));
    } catch (e) { 
      console.warn("Local registry update failed", e); 
    }

    // 2. Try to sync with Supabase (Best effort)
    try {
      const { error } = await withTimeout(
        supabase.from('users').update({ pin_code: newPin }).eq('id', editingUser.id) as any,
        10000,
        'updatePin'
      ) as any;
      if (error) {
          console.warn("Supabase PIN sync failed (likely RLS):", error.message);
          alert(`Supabase PIN sync failed: ${error.message}`);
      }
    } catch (e: any) {
      console.warn("Supabase PIN sync exception:", e.message);
      alert(`Supabase PIN sync exception: ${e.message}`);
    }

    // 3. Finalize
    alert(t('PIN updated successfully (Local)'));
    setEditingUser(null);
    setNewPin('');
    await fetchUsers();
    setSavingPin(false);
  ...
  if (!canManage) {
    return (
        <div className="p-4 bg-surface border border-border rounded-xl text-center text-sm text-secondary">
            <Lock size={16} className="mx-auto mb-2 opacity-50"/>
            {t('Administrator or Manager access required to manage users')}
        </div>
    );
  }
  ...
  return (
    <div className="bg-background/50 rounded-xl border border-border p-4 space-y-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <button onClick={fetchUsers} className="ml-auto p-2 rounded-lg border border-border hover:bg-surface text-secondary hover:text-text-main">
      ...
      {errorMsg && users.length > 0 && (
        <div className="text-xs text-amber-500 bg-amber-500/10 border border-amber-500/20 rounded-lg px-3 py-2">
          Không thể đồng bộ danh sách từ server, đang dùng dữ liệu local PIN.
        </div>
      )}
            ...
            {loading && users.length === 0 ? (
                ...
                <tr>
                  <td colSpan={4} className="p-4 text-center">
                    <div className="bg-red-500/10 text-red-500 p-3 rounded-lg text-sm inline-block">
                      <AlertTriangle size={16} className="inline mr-2" />
                      {errorMsg}
                    </div>
                    <div className="mt-2">
                      <button onClick={fetchUsers} className="text-primary hover:underline text-sm font-bold">Thử lại</button>
                    </div>
                  </td>
                </tr>
            ) : users.length === 0 ? (
                <tr>
                  <td colSpan={4} className="p-4 text-center text-secondary text-sm">
                    Chưa có user. Vui lòng tạo user hoặc đồng bộ.
                  </td>
                </tr>
            ) : users.map(user => (
...
const SettingsListRenderer: React.FC<{ 
  ...
  const renderControl = (item: SettingItemConfig) => {
    ...
    switch (item.type) {
      ...
      case 'custom-user-management': return <UserPinManagement isAdmin={isAdmin} isManager={isManager} />;
      default: return null;
    }
  };
...
export const Settings: React.FC<{ onLogout: () => void }> = ({ onLogout }) => {
  ...
  if (!can(role, PERMISSIONS.SETTINGS_VIEW)) {
      return (
          <div className="flex flex-col items-center justify-center h-full text-secondary">
              <ShieldAlert size={48} className="mb-4 opacity-50" />
              <h2 className="text-xl font-bold">Access Denied</h2>
              <p>You do not have permission to view settings.</p>
              <button onClick={onLogout} className="mt-4 px-4 py-2 bg-surface border border-border rounded-lg font-bold hover:bg-border">
                  Back to Login
              </button>
          </div>
      );
  ...
  const VIEW_CONFIGS = useMemo((): Record<string, SettingCardConfig> => {
    ...
    quick: {
      ...
      description: 'Các mục quan trọng nhất để hệ thống chạy ổn định',
      icon: LayoutTemplate,
      sections: [
        {
          title: 'Thiết lập vận hành nhanh',
          items: [
            { id: 'presetGrid', label: 'Áp dụng nhanh mô hình', type: 'preset-grid' },
            { id: 'bankSetup', label: 'Thiết lập VietQR', type: 'custom-bank-setup' },
            { id: 'testPrint', label: 'Test in', subtitle: 'Kiểm tra hóa đơn trước giờ mở bán', type: 'button', actionId: 'test_print' },
            { id: 'syncNow', label: combinedValues.networkStatusLabel === 'Offline' ? t('Đang Offline') : t('Đồng bộ ngay'), subtitle: 'Đẩy đơn chờ lên server', type: 'button', actionId: 'sync_now', disabled: combinedValues.networkStatusLabel === 'Offline' },
          ]
        },
        {
          title: 'Bảo mật bắt buộc',
          items: [
            { id: 'userPinManagement', label: 'User PIN Management', type: 'custom-user-management' },
            { id: 'managerRequirePinCancelOrder', label: 'Manager phải nhập PIN khi hủy order', type: 'toggle', valueKey: 'managerRequirePinCancelOrder' },
            { id: 'managerRequirePinDiscount', label: 'Manager phải nhập PIN khi giảm giá', type: 'toggle', valueKey: 'managerRequirePinDiscount' },
          ]
        }
      ]
    },
    ...
    advanced: {
      id: 'advanced',
      title: 'Nâng cao',
      description: 'Giao diện, báo cáo và các tinh chỉnh nâng cao',
      icon: Terminal,
      sections: [
        {
          title: 'Giao diện cơ bản',
          items: [
            { id: 'theme', label: 'Chế độ hiển thị', type: 'select', valueKey: 'theme', options: [
              { label: 'Hệ thống (System)', value: 'system' },
              { label: 'Tối (Dark)', value: 'dark' },
              { label: 'Sáng (Light)', value: 'light' },
            ] },
            { id: 'language', label: 'Ngôn ngữ', type: 'select', valueKey: 'language', options: [{label: 'Tiếng Việt', value: 'vi'}, {label: 'English', value: 'en'}] },
            { id: 'uiDensity', label: 'Mật độ giao diện', type: 'select', valueKey: 'uiDensity', options: [{ label: 'Thoáng', value: 'spacious' }, { label: 'Vừa', value: 'medium' }, { label: 'Dày (Compact)', value: 'compact' }] },
          ]
        },
        {
          title: 'Kiểm soát báo cáo',
          items: [
            { id: 'reportAccess', label: 'Ai được xem báo cáo', type: 'select', valueKey: 'reportAccess', options: [{ label: 'Chỉ chủ quán', value: 'owner' }, { label: 'Chủ + quản lý', value: 'manager' }, { label: 'Tất cả nhân viên', value: 'all' }] },
            { id: 'hideRevenue', label: 'Ẩn doanh thu chi tiết', type: 'toggle', valueKey: 'hideRevenue' },
            { id: 'allowExport', label: 'Cho phép xuất dữ liệu', type: 'toggle', valueKey: 'allowExport' },
          ]
        }
      ]
    },
  ...
  const visibleCards = useMemo(() => {
    if (role === 'admin') {
      return [
        VIEW_CONFIGS.quick,
        VIEW_CONFIGS.store,
        VIEW_CONFIGS.printing,
        VIEW_CONFIGS.staff,
        VIEW_CONFIGS.system,
        VIEW_CONFIGS.advanced,
      ];
    }
    ...
    return [VIEW_CONFIGS.quick, VIEW_CONFIGS.advanced];
  }, [role, VIEW_CONFIGS]);
