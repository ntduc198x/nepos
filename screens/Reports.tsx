...
const ReportsCharts = React.lazy(() => import('./ReportsCharts').then(m => ({ default: m.ReportsCharts })));
import { useCurrency } from '../CurrencyContext';
import { useTheme } from '../ThemeContext';
import { useData } from '../context/DataContext';
import { enrichOrderDetails, getTableLabel } from '../utils/orderHelpers';
import { printOrderReceipt } from '../services/printRuntime';
...
export const Reports: React.FC = () => {
  ...
  const reportCacheKey = useMemo(() => `reports_cache:${fromDate}:${toDate}:${role || 'guest'}:${user?.id || 'anon'}`, [fromDate, toDate, role, user?.id]);
  ...
  useEffect(() => {
    if (role === 'manager') {
      withTimeout(supabase.from('users').select('id').eq('role', 'admin'), 10000, 'fetchAdmins')
        .then(({ data }) => {
          if (data) setAdminIds(new Set(data.map(u => u.id)));
        })
        .catch(err => console.warn('[Reports] fetch admins failed:', err));
    }
  }, [role]);
  ...
  useEffect(() => {
    const handleOrderCompleted = () => setReloadTick(v => v + 1);
    window.addEventListener('pos:order:completed', handleOrderCompleted as EventListener);
    return () => window.removeEventListener('pos:order:completed', handleOrderCompleted as EventListener);
  }, []);

  // --- MAIN FETCH EFFECT ---
  useEffect(() => {
    fetchingRef.current = false; // Reset khi deps thay đổi
    const run = async () => {
        if (!canViewReport || !fromDate || !toDate) return;
        if (fetchingRef.current) return;
        fetchingRef.current = true;

        // Render cached snapshot immediately to avoid long blank/spinner on slow network.
        try {
            const cached = localStorage.getItem(reportCacheKey);
            if (cached) {
                const parsed = JSON.parse(cached);
                if (Array.isArray(parsed)) setReportData(parsed as Order[]);
            }
        } catch (e) {
            console.warn('[Reports] invalid cache, ignore:', e);
        }
        ...
        try {
            ...
            try {
                await withTimeout(
                    refreshOrdersForReportsRef.current(start.toISOString(), end.toISOString()),
                    12000,
                    'refreshOrdersForReports'
                );
            } catch (refreshErr) {
                console.warn('[Reports] refreshOrdersForReports failed/timeout, falling back to local data:', refreshErr);
            }

            const data = await withTimeout(
                getReportOrdersRef.current({ 
    ...
    run();
    return () => { fetchingRef.current = false; }; // ← Cleanup khi unmount
  }, [fromDate, toDate, canViewReport, reportCacheKey, reloadTick]);
                    ...
                    <>
                        {activeTab === 'overview' && (
                            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <Suspense fallback={<div className="xl:col-span-2 h-[560px] rounded-2xl border border-border bg-surface/40 animate-pulse" />}>
                                    <ReportsCharts
                                        hourlyData={hourlyData}
                                        topItemsData={topItemsData}
                                        t={t}
                                        maskedFormatPrice={maskedFormatPrice}
                                    />
                                </Suspense>
