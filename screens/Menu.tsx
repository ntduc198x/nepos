...
import { printOrderReceipt, isSandboxed, generateReceiptHTML } from '../services/printRuntime';
...
export const Menu: React.FC = () => {
  ...
  useEffect(() => {
    const computeInset = () => {
      ...
      // Difference between layout viewport and visual viewport indicates system UI inset.
      // Ignore extreme values (typically virtual keyboard) to keep CTA position stable.
      const rawInset = Math.max(0, Math.round(window.innerHeight - (vv.height + vv.offsetTop)));
      const normalizedInset = rawInset > 120 ? 0 : rawInset;
      setDeviceBottomInset(normalizedInset);
    };
