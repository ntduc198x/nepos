...
interface ExtendedContextValue {
  ...
  verifyMasterPin: (pin: string) => Promise<{ ok: boolean; reason?: string }>;
}
...
export const SettingsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const guardSensitive = useCallback(async (
    ...
    if (userRole === 'admin') {
        ...
        if (pinRequired) {
            ...
            const result = await SettingsService.verifyPin(input, user, userRole);
            
            if (!result.ok) {
                console.log(`[PIN_MODAL] Wrong PIN (Local Check Failed).`);
  ...
  const verifyMasterPin = useCallback(async (pin: string) => SettingsService.verifyPin(pin, user, userRole), [user, userRole]);
