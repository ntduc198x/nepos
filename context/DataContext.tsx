...
const DataProviderInner: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Timeout ref: tránh loading stuck vô hạn khi mạng không phản hồi
  ...
  // --- REALTIME SUBSCRIPTION FOR ORDERS + ORDER_ITEMS ---
  useEffect(() => {
    if (!user?.id || !isOnline) return;
          ...
          if (payload.eventType === 'INSERT' || payload.eventType === 'UPDATE') {
            ...
            if (row?.id) {
              await db.orders.put({
                ...
                staff: row.staff_name || 'POS',
                sync_status: 'synced'
              });
            }
          } else if (payload.eventType === 'DELETE') {
            const id = (payload.old as any)?.id;
            if (id) await db.orders.delete(id);
          }
        } catch (e) {
          console.error('[Realtime] orders apply failed:', e);
        }
      })
      .subscribe();
    ...
    const itemsChannel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'order_items' }, async (payload) => {
        try {
          if (payload.eventType === 'INSERT' || payload.eventType === 'UPDATE') {
            const row = payload.new as any;
            if (row?.id) await db.order_items.put(row);
          } else if (payload.eventType === 'DELETE') {
            const id = (payload.old as any)?.id;
            if (id) await db.order_items.delete(id);
          }
        } catch (e) {
          console.error('[Realtime] order_items apply failed:', e);
        }
      })
      .subscribe();
  ...
  const pullOrdersRange = async ({ fromISO, toISO, statusFilter }: { fromISO?: string, toISO?: string, statusFilter?: string[] }) => {
    ...
    try {
        ...
        // RBAC filtering
        if (role === 'staff' && user) {
            query = query.or(`staff_name.eq."${user.email.replace(/"/g, '\\"')}",user_id.eq.${user.id}`);
        }

        const { data: serverOrders, error } = await withTimeout(query as any, 12000, 'pullOrdersRange:orders') as any;
        ...
        const { data: itemsChunk, error: itemsError } = await withTimeout(
            supabase.from('order_items').select('*').in('order_id', chunk) as any,
            12000,
            'pullOrdersRange:order_items'
        ) as any;
  ...
  const refreshData = useCallback(async () => {
    const now = Date.now();
    // BUGFIX A: Throttle check ở ĐÂY, trước setLoading(true)
    // Nếu return sớm ở đây thì loading không bị thay đổi → không kẹt
    if (now - lastRefreshTime.current < 2000) return;
    lastRefreshTime.current = now;

    // Dọn timeout cũ nếu hàm được gọi lại
    if (refreshTimeoutRef.current) clearTimeout(refreshTimeoutRef.current);

    setLoading(true);

    // BUGFIX B: Timeout 20 giây - nếu Supabase không phản hồi sẽ tự tắt loading
    // Ngăn Dashboard quay spinner vô hạn khi mạng chậm hoặc Supabase down
    refreshTimeoutRef.current = setTimeout(() => {
      console.warn('[DataContext] refreshData timeout 20s - buộc tắt loading');
      setLoading(false);
    }, 20000);
    ...
    try {
      ...
      const [menuRes, tablesRes] = await Promise.all([
        withTimeout(supabase.from('menu_items').select('*').order('name') as any, 12000, 'refreshData:menu_items'),
        withTimeout(supabase.from('tables').select('*') as any, 12000, 'refreshData:tables')
      ]) as any[];
      ...
      await db.transaction('rw', ['menu_items', 'pos_tables'], async () => {
        if (menuRes && menuRes.data) {
          ...
          await db.menu_items.bulkPut([...menuRes.data.map((i: any) => ({...i, sync_status: 'synced'})), ...localItems]);
        }
      ...
      // Do not block first render on long dashboard pull; sync in background.
      refreshOrdersForDashboard().catch((e) => {
        console.warn('[DataContext] background refreshOrdersForDashboard failed:', e);
      });
      
    } catch (e: any) { 
        console.error("refreshData failed:", e); 
    } finally { 
        // Hủy timeout nếu fetch hoàn thành trước 20 giây
        if (refreshTimeoutRef.current) clearTimeout(refreshTimeoutRef.current);
        setLoading(false); 
    }
  }, [processQueue, user]);
  ...
  const addLocalOrder = async (data: any) => {
    ...
    // Inject store_id (default for now, or from settings)
  ...
  const checkoutSession = async (orderId: string, tableId: string, method: string, discountInfo?: { amount: number, type: 'percent' | 'amount', value: number }, paymentAmount?: number) => {
    ...
    // Notify report views immediately (local-first), then push to server right away when online.
    window.dispatchEvent(new CustomEvent('pos:order:completed', { detail: { orderId, updated_at: updates.updated_at } }));

    if (navigator.onLine) {
      try {
        await withTimeout(processQueue(true), 10000, 'checkoutSession:processQueue');
      } catch (e) {
        console.warn('[checkoutSession] immediate sync failed; will retry in background:', e);
      }
    }
  };
 ...
 const deleteMenuItem = async (id: any) => {
  ...
  const getReportOrders = async (range: { from: string, to: string }, opts?: { limit?: number }) => {
    ...
    try {
      ...
      const { data: serverData, error } = await withTimeout(query as any, 12000, 'getReportOrders:server') as any;
      if (error) throw error;
  ...
  const splitOrder = async (sourceOrderId: string, itemsToMove: { itemId: string; quantity: number }[], targetTableId: string) => {
    ...
    const storeId = 'default'; // Fallback
