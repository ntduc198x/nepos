...
const safeReadAutoSync = (): boolean => {
  try {
    const raw = localStorage.getItem(SETTINGS_STORAGE_KEY);
    if (!raw) return false;
    const parsed = JSON.parse(raw);
    return !!parsed?.data?.autoSync;
  } catch (e) {
    console.warn('[NetworkContext] Invalid settings JSON in localStorage. Ignoring autoSync flag.', e);
    return false;
  }
};
...
const SYNC_HANDLERS: Record<QueueActionType, (data: any) => Promise<void>> = {
  ...
  'tax_config_sync': async (data) => {
    const { config } = data;
    const { error } = await supabase.from('tax_configs').upsert(config, { onConflict: 'store_id' });
    if (error) throw error;
    console.log(`[SYNC_TAX_CONFIG] OK`);
  },

  'tax_closing_sync': async (data) => {
    const { closing } = data;
    const { error } = await supabase.from('tax_period_closings').insert([closing]);
    if (error) {
       if (isConflictError(error)) {
           console.log(`[SYNC_TAX_CLOSING] Conflict ignored`);
       } else {
           throw error;
       }
    }
    console.log(`[SYNC_TAX_CLOSING] OK`);
  },

  'tax_ledger_sync': async (data) => {
    const { entry } = data;
    const { error } = await supabase.from('tax_ledger_entries').insert([entry]);
    if (error) {
       if (isConflictError(error)) {
           console.log(`[SYNC_TAX_LEDGER] Conflict ignored`);
       } else {
           throw error;
       }
    }
    console.log(`[SYNC_TAX_LEDGER] OK`);
  },

  'expense_sync': async (data) => {
    const { entry } = data;
    const { error } = await supabase.from('expense_entries').insert([entry]);
    if (error) {
       if (isConflictError(error)) {
           console.log(`[SYNC_EXPENSE] Conflict ignored`);
       } else {
           throw error;
       }
    }
    console.log(`[SYNC_EXPENSE] OK`);
  }
};
