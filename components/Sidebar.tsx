...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView }) => {
  ...
  // Logic: Only show Inventory and Tax for non-staff roles
  ...
  return (
    <>
      ...
      {/* Profile Modal (omitted for brevity as it is unchanged logic) */}
      {/* ... keeping existing profile modal code logic ... */}
      {showProfileModal && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-surface border border-border rounded-2xl w-full max-w-sm shadow-2xl relative flex flex-col items-center p-6 lg:max-w-sm h-auto">
            ...
            <div
              className="size-32 rounded-full border-4 border-background shadow-xl mb-4 relative group overflow-hidden mt-4 cursor-pointer"
              onClick={() => fileInputRef.current?.click()}
            >
               <img src={previewUrl || avatarUrl || "https://ui-avatars.com/api/?name=User&background=random"} className="w-full h-full object-cover" alt="User Avatar"/>
               <div className="absolute inset-0 bg-black/45 opacity-100 md:opacity-0 md:group-hover:opacity-100 flex items-center justify-center transition-opacity">
            ...
            <div className="w-full space-y-3">
               <div className="w-full p-3 bg-background rounded-xl border border-border flex items-center gap-3">
                  ...
                  <div className="overflow-hidden">
                     ...
                     <input value={userName} onChange={(e) => setUserName(e.target.value)} className="text-sm text-text-main font-bold bg-transparent outline-none w-full border-b border-transparent focus:border-primary/50 transition-colors" placeholder="Enter name"/>
                  </div>
               </div>
               <button onClick={handleSaveChanges} disabled={isSaving} className="w-full py-3 bg-primary text-background rounded-xl font-bold hover:bg-primary-hover flex items-center justify-center gap-2 transition-colors">
